...
//	float domainPositions[numOfCorresp][2], rangePositions[numOfCorresp][2];//numOfCorresp
	...
	while(N > sampleCount){
		...
		if(IsGoodSample(domainPositions, numOfCorresp) &&
			...
			Mat A (Htmp->rows, Htmp->cols, CV_32F, Htmp->data.fl);
			cout << A << endl;
...
//float *meanB1 = new float[channels];
	...
	CvMatArray(A, aa, numOfCorresp * row, column);
	CvMatArray(U, u, numOfCorresp*row, numOfCorresp*row);
	CvMatArray(V, v, column, column);
	float eps = 0.000001;
	int i2;
