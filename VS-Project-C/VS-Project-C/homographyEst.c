...
int HomograhyEstimation2(CorspMap *inlierMap, CvMat2 *H)
{
	int i;
...
void RansacHomograhyEstimation(CorspMap *corspMap, CorspMap *inlierMap, CvMat2 *H)
{
	int numOfCorresp = 4;//4对互不共线的点
	//	float domainPositions[numOfCorresp][2], rangePositions[numOfCorresp][2];//numOfCorresp
	float(*domainPositions)[2] = (float(*)[2])malloc(sizeof(float) *numOfCorresp * 2);
	float(*rangePositions)[2] = (float(*)[2])malloc(sizeof(float) *numOfCorresp * 2);
	//float(*domainPositions)[2] = new float[numOfCorresp][2];
	//float(*rangePositions)[2] = new float[numOfCorresp][2];
	...
	CvMat2 *Htmp;
	Htmp = cvCreateMat2(3, 3, CV_32FC1);//创建中间临时量Htmp矩阵
	p = 0.99;
	e = 0.5;
	float N = 1000;
	int max_wait = 0;
	while (N > sampleCount) 
	{
		...
		for (i = 0; i < numOfCorresp; i++) 
		{
			pos = rand() % corspMap->len; // select random positions随机选择
										  // I -> y, J -> x
			domainPositions[i][1] = corspMap->domainImagePositionI[pos];//前一帧角点位置
			domainPositions[i][0] = corspMap->domainImagePositionJ[pos];
			rangePositions[i][1] = corspMap->rangeImagePositionI[pos];//后一帧角点位置
		...
		if (IsGoodSample(domainPositions, numOfCorresp) &&    //点对要满足非线性
			IsGoodSample(rangePositions, numOfCorresp)) 
		{
			// compute the homography 计算H矩阵
			ComputeHomography(domainPositions, rangePositions, numOfCorresp, Htmp);
			// calculate the distance for each correspondences  计算每个对应点对的距离
			// compute the number of inliers
			InitializeCorspMap(&tempInlierMap);
			CalculateDistance(Htmp, corspMap, &tempInlierMap); //计算距离
			...
			outlierProb = 1 - ((float)maxNumOfInliers / (float)totalNumOfPoints); //异常点个数占全部点数比例
			e = (e < outlierProb ? e : outlierProb); //即e取值小于等于0.5
...
/*		printf("请等待：%d...\n", sampleCount);
		...
		if (max_wait > 400)
		{
			printf("匹配时间过长放弃等待，H矩阵计算失败导致跟踪失败...\n");
			exit(0);
		}*/
	}
	free(domainPositions);
