...
//	Sobel(src, Dy, CV_32F, 0, 1, 3, scale, 0, BORDER_DEFAULT);
	...
	/*for (i = 0; i < size.height; i++)
	{
		...
		for (j = 0; j < size.width; j++)
		{
			printf("%f ", cov_data[j * 3]);   //第一个通道存dx*dx,即M矩阵左上角的元素  
			printf("%f ", cov_data[j * 3 + 1]);//第二个通道存dx*dy,即M矩阵左下角和右上角的元素  
			printf("%f \n", cov_data[j * 3 + 2]);//第三个通道存dy*dy,即M矩阵右下角的元素  
		}
	}*/
	//cout << cov << endl;
...
void calcMinEigenVal(const Mat& _cov, Mat& _dst)
{
	int i, j;
	Size size = _cov.size();
